...
for word in basic:
    list_basic.append(word.lower())

list_basic = sorted(list_basic)

# list_doc = []

# for word in w_obj:
#     list_doc.append(word.lower())

# list_doc = sorted(list_doc)

copy_obj = list_doc[:] # 复制最初的生词列表，以备统计生词率
...
# for word in shengci_list:
#     if len(word) < 4:
#         shengci_list.remove(word)

# for word in shuci_list:
#     if len(word) < 4:
#         shuci_list.remove(word)

# 展示


message = "\n总计单词（含重复）：" + str(len(list_doc)) + "\n" + \
    "生词（含重复）：" + str(len(shengci_list)) + "\n" + \
    "此处统计（含重复）的生词/全词比：" + \
    str(round(len(shengci_list)/len(list_doc)*100, 1)) + "%" + "\n"
...
shuci_list = set(shuci_list)

shuci_list = list(shuci_list)
